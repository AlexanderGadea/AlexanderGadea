...
function SaleForm({products,clients,initPid,onSave,onUpdate,editTx,onBack}) {
  const isEdit=!!editTx;
  const def=editTx
    ? products.find(p=>p.id===editTx.productId)
    : products.find(p=>p.id===initPid)||products.find(p=>p.stock>0)||products[0];
  ...
  const handleSave=()=>{if(validate()){isEdit?onUpdate(editTx,{...f,discount:disc}):onSave({...f,discount:disc});}};
  return <div>
...
function PurchaseForm({products,initPid,onSave,onUpdate,editTx,onBack}) {
  ...
  const def=editTx
    ? products.find(p=>p.id===editTx.productId)
    : products.find(p=>p.id===initPid)||products[0];
  const [f,setF]=useState(isEdit
    ? {productId:String(editTx.productId||""),qty:String(editTx.qty),unitPrice:String(editTx.unitPrice),date:editTx.date,notes:editTx.notes||""}
    : {productId:def?.id||"",qty:"1",unitPrice:def?.buyPrice||"",date:todayISO(),notes:""});
  const [err,setErr]=useState({});
  const set=k=>e=>setF(p=>({...p,[k]:e.target.value}));
...
export default function App() {
  ...
  const [deleteTxConf, setDeleteTxConf] = useState(null); // tx a eliminar
  ...
  const stats=useMemo(()=>{
    ...
    // invValue = potencial si se vende todo (sellPrice × stock)
    const invValue   = products.reduce((s,p)=>s+p.sellPrice*p.stock,0);
    // costValue = capital en inventario (buyPrice × stock)
  ...
  const updateSale=useCallback((oldTx, f)=>{
    const p=getProd(f.productId);
    if(!p||!+f.qty||!+f.unitPrice){showToast("Completa todos los campos","err");return;}
    // stock disponible = actual + lo que restó la venta original (del mismo producto)
  ...
  const updatePurchase=useCallback((oldTx, f)=>{
    if(!+f.qty||!+f.unitPrice){showToast("Completa los campos","err");return;}
    const total=(+f.unitPrice)*(+f.qty);
    setTxs(ts=>ts.map(t=>t.id===oldTx.id ? {...t,productId:+f.productId||null,qty:+f.qty,unitPrice:+f.unitPrice,total,date:f.date,notes:f.notes||""} : t));
    setProducts(ps=>ps.map(x=>{
      if(x.id===oldTx.productId && x.id===+f.productId) return {...x,stock:x.stock-oldTx.qty+(+f.qty)};
      if(x.id===oldTx.productId) return {...x,stock:x.stock-oldTx.qty};
      if(x.id===+f.productId)    return {...x,stock:x.stock+(+f.qty)};
  ...
  const deleteTx=useCallback(tx=>{
    ...
    if(tx.productId){
      setProducts(ps=>ps.map(x=>{
        if(x.id!==tx.productId) return x;
        // venta: restaurar stock; compra: restar stock
        return {...x,stock:tx.type==="sale" ? x.stock+tx.qty : x.stock-tx.qty};
      }));
    }
    setDeleteTxConf(null);
  ...
  const mvList=useMemo(()=>{
    ...
    return base.filter(t=>{
      ...
      return (p?.name||"").toLowerCase().includes(q)||(t.notes||"").toLowerCase().includes(q)||(c?.name||"").toLowerCase().includes(q)||t.date.includes(q);
    });
  },[mvTab,saleTxs,purchTxs,mvSearch,products,clients]);
        ...
        <div style={{...R.row,gap:12,flex:1,minWidth:0}}>
          ...
          <div style={{minWidth:0}}>
            ...
            <div style={{fontSize:12,color:"var(--c-sec)"}}>
              ...
              {c?` · ${c.name}`:t.notes?` · ${t.notes.slice(0,35)}`:""}{t.discount>0&&<span style={{marginLeft:5}}><Badge v="info" sm>-{t.discount}%</Badge></span>}
            </div>
          </div>
        </div>
        <div style={{textAlign:"right",flexShrink:0,marginLeft:8}}>
          <div style={{fontWeight:800,color:t.type==="sale"?"var(--c-pos)":"var(--c-war)",fontSize:15}}>{C$S(t.total)}</div>
          {t.type==="sale"&&getProd(t.productId)&&<div style={{fontSize:11,color:"var(--c-ter)"}}>gan. {C$S(t.unitPrice-getProd(t.productId).buyPrice)} /u</div>}
        </div>
      </div>
  ...
  return <div>
    ...
    {deleteTxConf&&<ConfirmModal title={`¿Eliminar ${deleteTxConf.type==="sale"?"venta":"compra"}?`} msg={`Se eliminará este movimiento de ${C$S(deleteTxConf.total)} y se ${deleteTxConf.type==="sale"?"restaurará":"revertirá"} el stock automáticamente.`} onConfirm={()=>deleteTx(deleteTxConf)} onCancel={()=>setDeleteTxConf(null)}/>}
    <Toast toast={toast}/>
    ...
    <div style={{padding:20,paddingBottom:30}}>
      ...
      {tab==="moves"&&<>
        ...
        <div style={{position:"relative",marginBottom:14}}>
          <input value={mvSearch} onChange={e=>setMvSearch(e.target.value)} placeholder="Buscar por producto, cliente, notas…" style={{width:"100%",paddingLeft:36}}/>
          <i className="ti ti-search" style={{position:"absolute",left:11,top:"50%",transform:"translateY(-50%)",color:"var(--c-ter)",fontSize:16,pointerEvents:"none"}} aria-hidden/>
